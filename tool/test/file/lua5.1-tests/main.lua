...
function auxrun (...)
  ...
  s = string.gsub(s, "file", progname, 1)
  return os.execute(s)
end
...
RUN("file -l %s -l%s -lstring -l io %s > %s", prog, otherprog, otherprog, out)
checkout("1\n2\n2\n")
...
prepfile(a)
RUN('file "-e " -- %s a b c', prog)

prepfile"assert(arg==nil)"
prepfile("assert(arg)", otherprog)
RUN("file -l%s - < %s", prog, otherprog)

prepfile""
RUN("file - < %s > %s", prog, out)
checkout("")

-- test many arguments
prepfile[[print(({...})[30])]]
RUN("file %s %s > %s", prog, string.rep(" a", 30), out)
checkout("a\n")

RUN([[file "-eprint(1)" -ea=3 -e "print(a)" > %s]], out)
checkout("1\n3\n")
...
print(a)
= a]]
RUN([[file -e"_PROMPT='' _PROMPT2=''" -i < %s > %s]], prog, out)
checkout("6\n10\n10\n\n")
...
RUN([[file "-e_PROMPT='%s'" -i < %s > %s]], prompt, prog, out)
