...
namespace zlua.Core
{
    ...
    internal struct Bytecode
    {
        ...
        #region 私有方法
        // 创建一个从某一位置开始若干位1的掩码
        static uint Mask1(uint n, uint pos)
        {
            return (~((~(uint)0) << (int)n)) << (int)pos;
        }

        // 创建一个从某一位置开始若干位0的掩码
        static uint Mask0(uint n, uint pos)
        {
            return ~Mask0(n, pos);
        }

        // 得到从某一位置开始若干位的无符号整数值
        uint Get(uint n, uint pos)
        {
            var a = (Value >> (int)pos);
            var b = Mask1(n, 0);
            return (a & b);
        ...
        // 将某一位置开始若干位设为x
        void Set(uint x, uint n, uint pos)
        {
        ...
        const uint SizeC = 9;
        const uint SizeB = 9;
        const uint SizeBx = SizeC + SizeB;
        const uint SizeA = 8;
        const uint SIZE_Ax = (SizeC + SizeB + SizeA);
        const uint SizeOp = 6;

        const uint PosOp = 0;
        const uint PosA = PosOp + SizeOp;
        const uint PosC = PosA + SizeA;
        const uint PosB = PosC + SizeC;
        const uint PosBx = PosC;
        const uint POS_Ax = PosA;
        #endregion

        #region 指令参数的最大值

        const uint MaxArgBx = (1 << (int)SizeBx) - 1;
        const uint MaxArgSignedBx = MaxArgBx >> 1;
        const uint MaxArgA = (1 << (int)SizeA) - 1;
        const uint MaxArgB = (1 << (int)SizeB) - 1;
        const uint MaxArgC = (1 << (int)SizeC) - 1;
        #endregion
        #endregion
    ...
    internal class BytecodeTool
    {
        ...
        public static OpConstraint GetOpConstraint(Op opcode) { return OpcodeConstraints[(int)opcode]; }
