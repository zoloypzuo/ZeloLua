...
//C++ TO C# CONVERTER NOTE: Classes must be named in C#, so the following class has been named AnonymousClass5:
	...
	public AnonymousClass5 s;
	public double nval;
  }
  public AnonymousStruct2 u = new AnonymousStruct2();
...
public class FuncState
{
  ...
  public upvaldesc[] upvalues = Arrays.InitializeWithDefaultInstances<upvaldesc>(DefineConstants.LUAI_MAXUPVALUES); // upvalues
  public ushort[] actvar = new ushort[DefineConstants.LUAI_MAXVARS]; // declared-variable stack
}
...
//C++ TO C# CONVERTER NOTE: The following #define macro was replaced in-line:
//ORIGINAL LINE: #define hasmultret(k) ((k) == VCALL || (k) == VVARARG)

//C++ TO C# CONVERTER NOTE: The following #define macro was replaced in-line:
//ORIGINAL LINE: #define getlocvar(fs, i) ((fs)->f->locvars[(fs)->actvar[i]])

//C++ TO C# CONVERTER NOTE: The following #define macro was replaced in-line:
//ORIGINAL LINE: #define luaY_checklimit(fs,v,l,m) if ((v)>(l)) errorlimit(fs,l,m)
